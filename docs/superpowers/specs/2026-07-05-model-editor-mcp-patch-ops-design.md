...
4. **Fail-closed validation gate (closes review Critical C2; unifies structural + semantic + normalization).** Re-express the mutated document as an import-payload string — `JSON.stringify({ importMode: document.session.importMode, workflows: document.session.workflows })`, the target workflow mutated in place — and run it through `ctx.parseImport` (the exact mechanism `update_workflow` uses on its caller content). This single pass performs all three concerns the two prior review rounds required: **(i) normalization** — `parseImportPayload` runs `normalizeOperatorAlias` (recursive `operatorType→operation`) and `normalizeWorkflowInput` (name/next/code trim) internally, so the tools accept exactly what `update_workflow` accepts (round-2 normalization-skew fix, for free); **(ii) structural validation** — the same zod schemas run, and `CriterionSchema` (a `z.literal`-discriminated union) rejects a malformed/unknown-`type` criterion as an `error` issue. **This is the C2 fix: the document is validated via `JSON.stringify`→`parseImport`, NOT via `serializeImport` — `serializeImport`'s `outputCriterion` would silently drop a malformed criterion before validation could see it, so it must never run before this gate**; **(iii) semantic validation** — `validateSemantics` catches dangling `next`/`initialState`, in-state duplicate names, `NAME_REGEX` violations, and missing-required `manual`. If `!parsed.document` or any `error`-severity issue → `VALIDATION_FAILED` via the shared helper (**write nothing**). *(C1 — lifecycle-`state` references — is NOT covered by any of this; it is handled explicitly in step 3's apply for `rename_state`/`remove_state`, §6.)*
5. **Write:** on success, `serializeImport(parsed.document)` (the clean, reparsed, canonical document) → `ctx.write(entry.relativePath, canonical)` (confined). Content-file only — the watcher live-pushes the change to the browser via the existing mechanism.
6. **Sidecar (state renames/removes only, §7):** best-effort, *after* the content write.
7. **Return** the op-specific success envelope with `diff` (= `jsonDiff(before-raw, canonical)`) + `diagnostics` (`parsed.issues`).

Two shared internals keep the six handlers thin: `loadWorkflowForEdit(ctx, workflow) → { entry, document, workflow, before }` (steps 1–2 + `NOT_FOUND`, throws the error envelope) and `commitEditedWorkflow(ctx, entry, document, before) → { path, diff, diagnostics }` (steps 4–5 + `diff`, throws the `VALIDATION_FAILED` envelope). The `VALIDATION_FAILED` envelope is produced by a shared `validationFailed(issues)` helper (§9). A shared `walkCriteria(criterion, visit)` helper traverses `group.conditions[]` **and** `function.function.criterion` (§2) for the C1 cascade/scan in step 3.
...
New Zod inputs in `server/schemas.ts` (all `.strict()`), each with a `z.infer` type export. A shared transition-body helper models the fields explicitly at the top level with **loose** nested passthroughs — the deep grammar is enforced at apply-time by the §5 step-4 re-parse (the same zod schemas `parseImport` runs, including the `z.literal`-discriminated `CriterionSchema`), not by the MCP input schema:
...
- **C2** — malformed `patch.criterion` silently dropped, written `ok:true` (`outputCriterion` returns `undefined` for unknown `type`; `validateAll` semantic-only). Fix: validate the mutated document by re-expressing it as a payload string and running `parseImport` (NOT `serializeImport`) so the zod `CriterionSchema` rejects it before any lossy serialize (§5 step 4). Regression-tested.
...
- **Normalization skew** — resolved for free by the §5 step-4 mechanism: the re-expressed document is run through `parseImport`, which internally applies `normalizeOperatorAlias` + identifier-trim, so the patch tools accept the same inputs `update_workflow` does (§2/§5 step 4). *(This unified re-parse gate — chosen during planning over the two-round two-gate description — realizes the same C2 + normalization + semantic guarantees the reviews required, via `update_workflow`'s own proven path.)*
