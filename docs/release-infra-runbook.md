...
Create an empty repo `Cyoda/homebrew-cyoda` (public). The cask is **generated by CI** — do not hand-write it.
...
## 3. Release-bot GitHub App (token to push the cask)

The cask is committed to the tap by a GitHub App, not a personal token. Reuse the existing app cyoda-go uses.

### 3.1 Identify the app
In `Cyoda` org → **Settings → Developer settings → GitHub Apps**, find the release-bot app (documented in cyoda-go's `MAINTAINING.md` as **`cyoda-platform-release-bot`** — confirm the exact name and note its numeric **App ID**). If it does not exist, create one: **New GitHub App**, owned by `Cyoda`, **Repository permissions → Contents: Read and write**, no webhook, "Only on this account". Generate and download a **private key** (`.pem`).

### 3.2 Install it on the tap
Install the app on **`Cyoda/homebrew-cyoda`** only (Settings → the app → Install → select that repo). Contents: Read and write. Confirm the app and the tap are under the same `Cyoda` account (the workflow mints with `owner: cyoda`).

### 3.3 Store the token credentials
The App ID is not sensitive (a **variable**); the key is a **secret**. Org-level so cyoda-go can share them:
...
```
This runs the whole pipeline including `publish-cask`, which validates notarization, regenerates `Casks/cyoda-dev-console.rb`, and commits it to `Cyoda/homebrew-cyoda` as `cyoda-go-release-bot`.
...
- [ ] `Casks/cyoda-dev-console.rb` was committed to the tap by `cyoda-go-release-bot` with correct per-arch SHAs.
...
1. **Tap consolidation:** retarget cyoda-go's GoReleaser `brews:` from `homebrew-cyoda-go` → `Cyoda/homebrew-cyoda` writing into `Formula/`; dual-publish to the old tap for ≥1 cycle; update its README/install docs to `cyoda/cyoda`. (Track as a coordination issue on `Cyoda/cyoda-go`.)
2. **Share the bot:** confirm `cyoda-platform-release-bot` + the `HOMEBREW_TAP_APP_ID`/`HOMEBREW_TAP_APP_KEY` org credentials resolve in cyoda-go's workflow.
