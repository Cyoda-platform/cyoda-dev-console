...
export const TOOL_MANIFEST: ToolManifestEntry[] = [
  { name: "list_workflows", description: "List discovered workflows: { workflows: [{ name, path, states, transitions, valid, reason? }] }. reason is present only when valid is false (first error-severity diagnostic's message, or a short parse-failure note) — use it to learn why a workflow is invalid without a separate validate_workflow call.", inputSchema: empty },
  ...
  { name: "optimize_layout", description: "Re-lay-out a workflow with elkjs and persist node positions to a .layout.json sidecar (also pushed live to the browser editor). options.orientation: \"vertical\" top-to-bottom (default) | \"horizontal\" left-to-right. options.preset: \"websiteCompact\" tight horizontal flow for docs embeds, \"configuratorReadable\" balanced vertical flow for editors, \"opsAudit\" spread layout for ops dashboards. options.nodeSize overrides the default per-state box size; options.pinned fixes specific node coordinates.", inputSchema: { type: "object", properties: { name: { type: "string", minLength: 1 }, options: { type: "object", properties: { orientation: { enum: ["vertical", "horizontal"] }, preset: { enum: ["websiteCompact", "configuratorReadable", "opsAudit"] }, nodeSize: { type: "object", properties: { width: { type: "number" }, height: { type: "number" } }, required: ["width", "height"], additionalProperties: false }, pinned: { type: "array", items: { type: "object", properties: { id: { type: "string" }, x: { type: "number" }, y: { type: "number" } }, required: ["id", "x", "y"], additionalProperties: false } } }, additionalProperties: false } }, required: ["name"], additionalProperties: false } },
  { name: "validate_workflow", description: "Parse + validate a workflow; returns diagnostics. Read-only.", inputSchema: nameOnly },
  { name: "validate_workflows", description: "Batch form of validate_workflow: validates every discovered workflow in one call. Read-only. { workflows: [{ name, valid, diagnostics }] } — one entry per discovered workflow; an unreadable/unparseable file yields valid:false with empty diagnostics rather than failing the whole call.", inputSchema: empty },
  { name: "connection_info", description: "Return the current browser URL/port for the live editor. The port is deterministic per project path (stable across restarts) but the token regenerates every process start — call this again after a server restart to get the fresh, valid URL.", inputSchema: empty },
  { name: "list_entities", description: "List discovered entities (separate plain-JSON object files matched by entityGlobs): { entities: [{ name, path, lastModified, sizeBytes }] }.", inputSchema: empty },
  ...
  { name: "get_project", description: "Report the project root, current workflowGlobs/entityGlobs, and workflow/entity counts. Also includes the connection URL under _connection.url (only get_project and connection_info ever return it).", inputSchema: empty },
];
